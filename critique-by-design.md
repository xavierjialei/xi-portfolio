...
With the redesigned visualization, I want to make it easier for readers to see the fact that the Chinese film market is growing and has been contributing more shares of box office sales in global movies like Marvel over the years. The chart starts with the data of the most recent movies as they are more relevant. All bars represent the sales numbers instead of percentages for accurate representation. Readers can still see the percentage shares in the pop-up window that is shown when the mouse hovers over the corresponding bar. The darker blue is used for the "sales from China" section of the bars for better contrast and highlight. It also guides readers to focus on the message the chart is trying to convey. Number labels are removed to provide a clear layout. A conclusive sentence is added as a subtitle of the chart to aid understanding so that readers don't necessarily need to spend too much effort going through the chart. In the future, I might place the years of each movie in a separate column and group the movies that are released in the same year. Flourish does not seem to allow me to add additional columns to a chart at this stage, but I believe there are ways to achieve this so that the timeline information can be further highlighted.
